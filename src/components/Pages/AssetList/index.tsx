...
const forbiddenTokens = ["Bonk", "USDT", "USDC"];
...
const AssetList: React.FC = () => {
  ...
  const [mintToken, setMintToken] = useState(USDT_TOKEN_MINT);
  ...
  const handleRadioChange = (id: number) => {
    ...
    const newMintToken = id === 1 ? USDT_TOKEN_MINT : USDC_TOKEN_MINT;

    setSelectedOption(id);
    setMintToken(newMintToken);
    updateDataForNewMintToken(newMintToken);
  };

  const updateDataForNewMintToken = async (mintToken: any) => {
    if (!walletAddress || !jupiterQuoteApi) {
      console.error("Wallet address or Jupiter API client is not available.");
      return;
    }

    console.log("Starting data update for new mint token:", mintToken);
    setState(ApplicationStates.LOADING);

    try {
      const assets = await getTokenAccounts(walletAddress, connection, tokens);
      const filteredAssets = assets.filter(
        (asset) => !forbiddenTokens.includes(asset.token.symbol)
      );

      const newAssetList = filteredAssets.reduce((acc: any, asset: any) => {
        const assetId = asset.token.address;
        acc[assetId] = new AssetState(asset);
        return acc;
      }, {});
              ...
              if (prevList[id]) {
                return {
                  ...
                  [id]: new AssetState(
                    ...
                    prevList[id].swap,
                    prevList[id].checked,
                    prevList[id].transactionState,
                    prevList[id].transactionId
                  ),
                };
              }
              return prevList;
      ...
      console.error("Failed to update data for new mint token:", error);
    }
  };
            ...
            <div>
              <ul className="flex flex-col sm:flex-row">
                {radioOptions.map((option) => (
                  <li
                    key={option.id}
                    className="w-[100%] inline-flex items-center gap-x-2 py-3 px-4 text-sm font-medium bg-white border text-gray-800 -mt-px first:rounded-t-full first:mt-0 last:rounded-b-full sm:-ml-px sm:mt-0 sm:first:rounded-tr-none sm:first:rounded-bl-full sm:last:rounded-bl-none sm:last:rounded-tr-full"
                  >
                    <div className="flex items-center">
                      <input
                        id={`horizontal-list-group-item-radio-${option.id}`}
                        type="radio"
                        ...
                        disabled={state === 0 ? true : false} // Disable input while fetching data
                        onChange={() => handleRadioChange(option.id)}
                      />
                      <label
                        htmlFor={`horizontal-list-group-item-radio-${option.id}`}
  ...
  return (
    <>
      <div className="flex flex-col gap-4 z-30 relative bg-[#160700] px-[80px] py-[100px]">
        <ToastContainer />
        <SummaryModal />
        {ScoopList()}
      </div>
